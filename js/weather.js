            ...
            .then(data => {
                ...
                if (weatherDescription.includes('light rain')) {
                    ...
                    if (new Date().getHours() >= 19 || new Date().getHours() < 7) { 
                        image.src = 'images/clear_sky_night.jpg'; //Nighttime image
                    } else { // 
                        image.src = 'images/clear_sky.jpg'; // Daytime image 
                    }
                    weatherHtml += image.outerHTML;
                } else if (weatherDescription.includes('overcast clouds') || weatherDescription.includes('scattered clouds') || weatherDescription.includes('broken clouds')) {
                    if (new Date().getHours() >= 19 || new Date().getHours() < 7) { 
                        image.src = 'images/cloudy_night.jpg'; // Cloudy night image 
                        weatherHtml += image.outerHTML;
                    } else { 
                        // Daytime cloud images
                    ...
                    weatherHtml += image.outerHTML; // Append the image
                    weatherHtml += '<p class="warning">Warning: Stay indoors! Postpone all activities for today</p>'; // Append the warning
                } else {
                    // TBD in the future
                }
